...
##########################################################################
HISTORY
version 3.0.0 - 2015-11-?? - Al C - first iteration with the new features
version 3.0.1 - 2016-03-05 - Al C - fixed consistency between dev & current; fixed missing hiddenDevices
version 3.1 - 2016-10-16 - Al C - various fixes and an update to the drop down for devices on the daily tab
version 3.1.2 - 2016-10-22 - Al C - added alert for out of date yamon3.x.html
				2016-10-25 - Al C - fixed update to config3.js; added version text, fixed sh: error on startup; fixed double slash in Ajax urls
version 3.2.5 - 2017-03-11 - Al C - changes to loadSettings and edit devices dialog
version 3.3.4 - 2017-09-03 - Al C - big version bump for consistency... adopted data-* for attributes
version 3.3.5 - 2017-11-12 - Al C - added router details, fixed issues
version 3.3.6 - 2017-12-9 - Al C - alert for missing data
version 3.3.7 - 2018-01-19 - Al C - alerts.js functionality
version 3.4.0 - 2018-02-04 - Al C - uprev'd for new version
version 3.4.5 - 2018-07-09 - Al C - uprev'd; lots of changes, ip by CIDR; updated dashboard tables, housekeeping
*/
...
var devices=[],orig_devices=[],names=[],monthly=[],monthly_totals=[],hourly=[],hourly_totals=[{down:0, up:0}],corrections=[],pnd_data={},p_pnd_d=0,p_pnd_u=0,p_dropped=0,p_local=0,o_sut,hourlyloads=[]
var months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']
var _unlimited_usage,_doLiveUpdates,_doCurrConnections,_updatefreq,_organizeData,freeMem,availMem, totMem
//var users_updated,hourly_updated
...
var serverloadoptions={width:475,height:300,chartArea: {width: '80%',height: '73%'},title:'Average Server Load',curveType:'function',fontSize:9,legend:{position:'top',fontSize:14},isStacked:true,hAxis:{title:'Time',fontSize:14,slantedText:true,titleTextStyle:{color:'green'}},vAxis:{title:'Load',fontSize:14,titleTextStyle:{color:'green'},viewWindow:{min:0}},series:{0:{lineWidth:1,color:'blue',visibleInLegend:true},1:{lineWidth:1,color:'green',visibleInLegend:true},2:{lineWidth:1,color:'red',visibleInLegend:true}}
};
var do_options={width:475,height:300,chartArea: {width: '80%',height: '73%'},title:'Live Downloads (in KB/s)',curveType:'function',fontSize:9,legend:{position:'top',fontSize:14},isStacked:true,hAxis:{title:'Time',fontSize:14,slantedText:true,titleTextStyle:{color:'green'}},vAxis:{title:'Usage in KB/s',fontSize:14,titleTextStyle:{color:'green'},viewWindow:{min:0}},series:{0:{lineWidth:1,color:'red',visibleInLegend:true},1:{lineWidth:1,color:'green',visibleInLegend:true}}}
var up_options={width:475,height:300,chartArea: {width: '80%',height: '73%'},title:'Live Uploads (in KB/s)',curveType:'function',fontSize:9,legend:{position:'top',fontSize:14},isStacked:true,hAxis:{title:'Time',fontSize:14,slantedText:true,titleTextStyle:{color:'green'}},vAxis:{title:'Usage in KB/s',fontSize:14,titleTextStyle:{color:'green'},viewWindow:{min:0}},series:{0:{lineWidth:1,color:'blue',visibleInLegend:true},1:{lineWidth:1,color:'purple',visibleInLegend:true}}}
...
$(document).ready(function (){
	...
	if (typeof(_html_version)=='undefined'){
		getMessage('no html version')
		_html_version='Out-of-date'
	}
	$('.html_version').text(_html_version)
	if(typeof(_scriptVersion)=='undefined'){
		getMessage('no _scriptVersion')
		clearLoading(6000)
	   return false
	   //_scriptVersion='Out-of-date'
	}
	$('.scriptVersion').text(_scriptVersion)
	if (typeof(_organizeData)=='undefined') {
		

		getMessage('no _organizeData')
		_organizeData=0
	}
	$( ".report-date" ).slider({
	...
	addISPList()
	maxGrWidth=Math.min(1000, $('.tab-div').css('width').replace('px','')-48)
	$('#bnDBtn').css('margin-left', maxGrWidth-144*2-32)
	freeMem=freeMem||0
	availMem=availMem||0
	totMem=totMem||0

	$('._detanod').replaceWith("<div class='_detanod' amount=20 title='Thank you in advance for your support!' ><div class='submenu'><span class='amount btn' amount=10 title='Donate $10'>$10</span><span class='amount btn' amount=15 title='Donate $15'>$15</span><span class='amount btn' amount=20 title='Wow... thanks!!!'>$20</span><span class='amount btn' amount=25 title='Wow... thanks!!!'>$25</span><span class='amount btn' amount='' title='Select this option to choose your own amount'>Other...</span></div></div>")
	...
	$.getScript( "js/router.js")
	.done(function(data){
		if(data.length==0){
			getMessage('null router.js')
			return
		}
		$('.router-brand').text(router)
		$('.router-firmware').text(firmware)
		$('.router-installed').text(installed)
		$('.router-updated').text(updated)
		if (!g_Settings['router']){
			uploadRouterJS('save router')
		}
		else if(g_Settings['router']['share']=='1' && updated!=g_Settings['router']['updated']){
...
function setViews(){
	if(!g_Settings['complete']==1){
		$('#tabs-w,#p-timer, #intro-p, #settings-tab-section>*').hide()
		$('.intro').removeClass('hidden')
		$('#settings-tab-section, .intro').show()
		$('.intro-t').first().siblings('.intro-t').hide()
		$('#intro-bw_cap').replaceWith($('#p-bw_cap').clone(true, true))
		$('#intro-bonus_cap').replaceWith($('#p-bonus_cap').clone(true, true))
		$('#settings-tab-section').addClass('bt')
		clearLoading(500)
		return false
	}
...
function loadDevices(){
	...
	var device_file=_wwwData+_usersFileName
	...
	function getMonthlyData(myr, mn, da){
		...
		.done(function(dlist,textStatus){
			...
			while (td!=rd && !monthly_totals.usage[dn].down && !monthly_totals.usage[dn].up){
				...
				var datafile=dp+yr+'-'+mo+'-'+da+'-'+_hourlyFileName
			...
			while (nn<today && nn<=_re_Date){
				...
				var datafile=dp+yr+'-'+mo+'-'+da+'-'+_hourlyFileName
				if(monthly_totals.usage[dn].down==0 && monthly_totals.usage[dn].up==0){
...
function loadHourly(cleardata){
	...
	for(var hr=0;hr<24;hr++){
		hourly_totals.usage[hr]={down:0,up:0}
	}
	...
	.done(function(dlist,textStatus){
		...
		saveSettings()
		/*$.each(monthly_totals.usage,function(n,m){
			console.log(n,m, monthly_totals.pnd)
			var p=monthly_totals.pnd[n]
			p.dn_d=p.down-m.down
			p.up_d=p.up-m.up
			p.dn_p=p.dn_d/m.down||0
			p.up_p=p.up_d/m.up||0
			p.t_p=(p.dn_d+p.up_d)/(m.down+m.up)||0
		})*/
...
function ud_a(arr){
	...
	if(!g_Settings['devices']||!g_Settings['devices'][mac]){
		...
		orig_devices[mac]={group:arr.owner,name:arr.name}
	}
...
function serverloads(minL,minTS,maxL,maxTS){
	minL*=1
	maxL*=1
	var gw=$('.gradient').css('width').replace('px','')*1 ,wm=gw/Math.max(_processors,$('#sp_maxSL .txt').text()*1)
	var los=Math.max(22,$('.gradient').position()['left']*1)
	$('#serverload').fadeIn('slow')
	$('#sp_minSL').attr('title','Min load at: '+minTS)
	$('#sp_minSL .txt').text(minL.toFixed(2))
	$('#sp_maxSL').attr('title','Max load at: '+maxTS)
	$('#sp_maxSL .txt').text(maxL.toFixed(2))
	var l1=Math.min(gw, minL*wm-Math.max(20,$('#sp_minSL').width())/2)+los,l2=Math.min(gw, maxL*wm-Math.max(20,$('#sp_maxSL').width())/2)+los
...
function serverload(l1,l5,l15){
	var gw=$('.gradient').css('width').replace('px','')*1, wm=gw/Math.max(_processors,$('#sp_maxSL').text()*1)
	var los=$('.gradient').position()['left']*1
	$('#sp_curSL').text(l5.toFixed(2)).attr('title','Current load')
	var l3=Math.min(gw, l5*wm-Math.max(20,$('#sp_curSL').width())/2)+los
...
function setDevices(){
	...
	Object.keys(devices).sort(byName).forEach(function(d){
		...
		if($('#dd-'+id).length==0){
			...
			nr.find('.deviceIP').text(devices[d].ip)
			nr.find('.deviceIPv6').text(devices[d].ip6)
...
function setACses(){
	$('<option/>').attr('value','').prop('disabled','disabled').text('start').appendTo('.ac-start')
	$('<option/>').attr('value','').prop('disabled','disabled').text('end').appendTo('.ac-end')
	for(var x=0;x<24;x++){
		var hr=twod(x)
		$('<option/>').attr('value',hr+':00').text(hr+':00').appendTo('.ac-start,.ac-end')
		$('<option/>').attr('value',hr+':30').text(hr+':30').appendTo('.ac-start,.ac-end')
	}
	$('#ac-gr-l').html('')
	Object.keys(names).forEach(function (k) {
		var gn=names[k].group,cgn=names[k].cg,tl=$('<label/>').attr('for','ac-gp-' + cgn)
		$('<input/>').attr('type','checkbox').attr('value',cgn).addClass('ac-gp').attr('id','ac-gp-' + cgn).appendTo(tl)
		tl.append(gn).appendTo(tl)
		tl.appendTo($('#ac-gr-l'))
	})
	$('.ac-gp').change(function () {
		clickCB(this)
	})
}
function y_a(m){
